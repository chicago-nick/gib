    ...
    LazyColumn(
        ...
        item {
            Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                ...
                Text("Compact rows first, spreadsheet detail on demand.", style = MaterialTheme.typography.bodyLarge)
            }
        }
        stickyHeader {
            Card(modifier = Modifier.fillMaxWidth()) {
                Row(
                    ...
                    Row {
                        ...
                        TextButton(onClick = onCollapseAll) { Text("Collapse") }
                    }
                }
            }
        }
        items(state.rows, key = { it.date.toEpochDay() }) { row ->
            ...
            SectionCard(
                ...
                subtitle = if (isExpanded) "Tap to collapse" else "Tap for meal detail. Long-press for edit.",
